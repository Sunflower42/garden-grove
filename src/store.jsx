...
import { setCustomPlantsCache } from './data/plants';
...
const initialState = {
  ...
  // User-created plants — extends the built-in PLANTS list
  customPlants: [], // [{ id, name, category, emoji, custom: true, ...plant fields }]
...
function loadState() {
  try {
    ...
    if (saved) {
      ...
      setCustomPlantsCache(merged.customPlants || []);
      return merged;
    }
  } catch (err) {
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'ADD_CUSTOM_PLANT': {
      // payload: a full plant object — id is generated by the caller
      return { ...state, customPlants: [...(state.customPlants || []), action.payload] };
    }
    case 'REMOVE_CUSTOM_PLANT': {
      return { ...state, customPlants: (state.customPlants || []).filter(p => p.id !== action.payload) };
    }
...
export function StoreProvider({ children }) {
  ...
  // Sync custom plants into the plants.js module-level cache so getPlantById finds them
  useEffect(() => {
    setCustomPlantsCache(state.customPlants || []);
  }, [state.customPlants]);
